...
import pl.misern.taskm.event.command.task.RemovePersonTaskCommand;
import pl.misern.taskm.event.command.task.FinishPersonTaskCommand;
...
public class ShowTaskListDialogCommand implements Command {
	...
	private void registerDialogEventListeners() {
		AssignedTasksDialog tasksDialog = Window.getInstance().getAssignedTasksDialog();
